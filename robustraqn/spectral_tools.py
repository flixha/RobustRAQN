...
from pyfftw.interfaces import scipy_fft # these fft-functions return complex ns
...
def balance_noise(self, inv, balance_power_coefficient=2,
    ...
    """
    if len(self.data) == 0:
        Logger.warning('Cannot balance trace by noise PDF, there is no data for '
                     + str(self))
        return self
    else:
    ...
    # TODO: once Obspy has FrequencyTrace classes implemented, use these instead of
    #       manually converting to frequency domain
...
def sum_station_pdf(inv, pdf_dir, network, station, location="*",
    ...
    pdf = calcMegaPDF(freq_u, freq_u_str, db_u, pdffiles, outpdffile=outpdffile)
...
def calcMegaPDF(freq_u, freq_u_str, db_u, pdffiles, outpdffile='megapdf.npy'):
    ...
    for infile in pdffiles:
        try:
            ...
            Logger.warning(
                'NSLC time period {} not available, so I cannot check NLNM.'\
                .format(infile))
            continue
...
class Noise_model:
    ...
    def __init__(self, frequencies, decibels):
        ...
        :param decibels: decibel values for the spectral power of the 
                            displacement values, e.g. in units of
                            10log(m^2/s^4/Hz)[dB]; or in relative units.
        """
    ...
    for net in inv.networks:
        for sta in net.stations:
            if wcmatch.fnmatch.fnmatch(net.code, network) and\
                    wcmatch.fnmatch.fnmatch(sta.code, station,
                                            flags=wcmatch.fnmatch.EXTMATCH):
...
def attach_noise_models(inv, pdf_dir, outfile='inv.pickle',
    ...
    # array_list is a list of tuples, with the first tuple element containing the
    # array-prefix and the 2nd tuple element containing a list of stations at the
    # array.
    seisarray_list = list()
    ...
    for j, seisarray in enumerate(seisarray_list):
        ...
        try:
            ...
            Logger.warning('Cannot add noise model for array '
                           + seisarray_prefix)
    ...
    for station in single_station_list:
        try:
            ...
            Logger.warning('Cannot add noise model for station ' + station)
...
def plot_all_station_pdfs():
    ...
    for file in glob.glob('StationPDFs/*.npy'):
        try:
            ...
            Logger.warning(err)
            Logger.warning('Could not load file ' + file + ', continuing with next station')
            continue
...
#         """
#         read_ispaq_stats(folder, networks=['??'], stations=['*'],
